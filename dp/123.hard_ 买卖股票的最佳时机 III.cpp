...
k = 2 和前面题目的情况稍微不同，因为上面的情况都和 kk 的关系不太大。要么 kk 是正无穷，状态转移和 kk 没关系了；要么 k = 1，跟 k = 0 这个 base casebasecase 挨得近，最后也没有存在感。

这道题 k = 2 和后面要讲的 kk 是任意正整数的情况中，对 kk 的处理就凸显出来了。我们直接写代码，边写边分析原因。
...
这里 kk 取值范围比较小，所以可以不用 forfor 循环，直接把 k = 1 和 2 的情况手动列举出来也可以：
